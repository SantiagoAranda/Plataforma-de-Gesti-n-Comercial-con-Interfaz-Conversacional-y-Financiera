...
describe('Payroll Payments & Settlements (Integration)', () => {
  ...
  beforeAll(async () => {
    ...
    const globalParam = await prisma.payrollGlobalParameter.upsert({
      where: { legalCode: 'CO-PAYROLL-2026-V1' },
      update: {},
      create: {
    ...
    await prisma.payrollOvertimeRate.createMany({
      data: payrollRateFixtureData(globalParam.id),
      skipDuplicates: true,
    });
    ...
    await prisma.pucCuenta.upsert({
      where: { code: '5105' },
      create: { code: '5105', name: 'Gastos de personal', grupoCode: '51' },
      update: {},
    });
    ...
    await prisma.pucClase.upsert({
      where: { code: '2' },
      create: { code: '2', name: 'Pasivo' },
      update: {},
    });
    await prisma.pucGrupo.upsert({
      where: { code: '25' },
      create: { code: '25', name: 'Obligaciones laborales', claseCode: '2' },
      update: {},
    });
    await prisma.pucCuenta.upsert({
      where: { code: '2505' },
      create: { code: '2505', name: 'Salarios por pagar', grupoCode: '25' },
      update: {},
    });
    await prisma.pucSubcuenta.upsert({
      where: { code: '250505' },
    ...
    await prisma.pucClase.upsert({
      where: { code: '1' },
      create: { code: '1', name: 'Activo' },
      update: {},
    });
    await prisma.pucGrupo.upsert({
      where: { code: '11' },
      create: { code: '11', name: 'Disponible', claseCode: '1' },
      update: {},
    });
    await prisma.pucCuenta.upsert({
      where: { code: '1110' },
      create: { code: '1110', name: 'Bancos', grupoCode: '11' },
      update: {},
    });
    await prisma.pucSubcuenta.upsert({
      where: { code: '111005' },
    ...
    await prisma.payrollAccountingMapping.createMany({
      data: [
        ...
        {
          businessId,
          conceptCode: 'PAYROLL_PAYMENT_BANK',
          conceptName: 'Pago en Banco',
          side: 'CREDIT',
          accountCode: '111005',
          accountName: 'Bancos',
          isActive: true,
  ...
  describe('Payroll Payments Requirements', () => {
    ...
    it('1. Create BIWEEKLY contract with monthly payroll period', async () => {
      ...
      const contract = await payrollService.createContract(
        businessId,
        employeeId,
        {
    ...
    it('2. Automatic creation of two PayrollPayments within the same PayrollRun', async () => {
      ...
      const payments = await payrollService.listPayrollRunPayments(
        businessId,
        runId,
      );
    ...
    it('8. Mark payment PAID must not recalculate payroll', async () => {
      const runBefore = await prisma.payrollRun.findUnique({
        where: { id: runId },
      });
    ...
    it('7. Posting payroll must not mark payments as paid', async () => {
      ...
      const run2 = await prisma.payrollRun.findFirst({
        where: { payrollPeriodId: period2.id, employeeId },
      });
    ...
    it('10. Editing contract with POSTED payroll must block critical fields', async () => {
      ...
      await expect(
        payrollService.updateContract(businessId, contractId, {
    ...
    it('4. Retiro del empleado día 16 con Pago 1 PAID & 5. Liquidacion debe descontar Pago 1 & 6. Prima descontada', async () => {
      ...
      const run3 = await prisma.payrollRun.findFirst({
        where: { payrollPeriodId: period3.id, employeeId },
      });

      const payments3 = await prisma.payrollPayment.findMany({
        where: { payrollRunId: run3!.id },
      ...
      await payrollService.createContractBenefitPayment(
        businessId,
        contractId,
        {
          type: 'PRIMA',
      ...
      const simulation: any = await payrollService.simulateContractSettlement(
        businessId,
        contractId,
        {
          endDate: '2026-03-16',

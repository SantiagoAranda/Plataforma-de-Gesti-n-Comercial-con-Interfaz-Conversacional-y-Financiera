import {
  Prisma,
  PrismaClient,
  PayrollOvertimeCalculationMode,
  PayrollWithholdingStatus,
  UnitKind,
} from '@prisma/client';
...
async function seedSimpleTaxPucAccounts() {
  ...
  if (!cashAccount || !bankAccount) {
    throw new Error(
      'Simple tax payment accounts 110505 and 111005 must exist in PUC seed data',
    );
  }
  ...
  const byCode = Object.fromEntries(
    (
      await prisma.unit.findMany({
        where: { code: { in: units.map((unit) => unit.code) } },
      })
...
async function seedPayrollGlobalParameters(base: string) {
  ...
  for (const r of rows) {
    await prisma.payrollGlobalParameter.upsert({
      where: { legalCode: r.legal_code },
      // Seeds initialize missing data. Existing legal records are upgraded
      // by migrations and are intentionally preserved on repeated seeds.
      update: {},
      create: {
        ...
        effectiveFrom: utcDate(r.effective_from)!,
        effectiveTo: utcDate(r.effective_to),
...
async function seedOvertimeRates(base: string) {
  ...
  for (const r of rows) {
    const globalParameter = await prisma.payrollGlobalParameter.findUnique({
      where: { legalCode: r.legal_code },
    });
    if (!globalParameter) {
      throw new Error(`No existe PayrollGlobalParameter ${r.legal_code}`);
    }

    const percentageFromFactor = (factor: string) =>
      new Prisma.Decimal(factor).sub(1).mul(100).toString();
    const nightSurcharge = toDecimal(r.night_surcharge)!;
    const sundaySurcharge = toDecimal(r.sunday_holiday_surcharge)!;
    const rates = [
      {
        code: 'HORA_ORDINARIA_NOCTURNA',
        name: 'Hora Ordinaria Nocturna',
        legalPercentage: new Prisma.Decimal(nightSurcharge).mul(100).toString(),
        totalFactor: new Prisma.Decimal(1).add(nightSurcharge).toString(),
        payableMultiplier: nightSurcharge,
        calculationMode: PayrollOvertimeCalculationMode.SURCHARGE_ONLY,
      },
      {
        code: 'HORA_EXTRA_DIURNA',
        name: 'Hora Extra Diurna',
        legalPercentage: percentageFromFactor(r.day_overtime_factor),
        totalFactor: toDecimal(r.day_overtime_factor)!,
        payableMultiplier: toDecimal(r.day_overtime_factor)!,
        calculationMode: PayrollOvertimeCalculationMode.FULL_HOUR_FACTOR,
      },
      {
        code: 'HORA_EXTRA_NOCTURNO',
        name: 'Hora Extra Nocturno',
        legalPercentage: percentageFromFactor(r.night_overtime_factor),
        totalFactor: toDecimal(r.night_overtime_factor)!,
        payableMultiplier: toDecimal(r.night_overtime_factor)!,
        calculationMode: PayrollOvertimeCalculationMode.FULL_HOUR_FACTOR,
      },
      {
        code: 'HORA_DOMINICAL_FESTIVO',
        name: 'Hora Dominical/Festivo',
        legalPercentage: new Prisma.Decimal(sundaySurcharge)
          .mul(100)
          .toString(),
        totalFactor: toDecimal(r.sunday_holiday_factor)!,
        payableMultiplier: sundaySurcharge,
        calculationMode: PayrollOvertimeCalculationMode.SURCHARGE_ONLY,
      },
      {
        code: 'HORA_EXTRA_DOM_FESTIVO',
        name: 'Hora Extra Dom/Festivo',
        legalPercentage: percentageFromFactor(
          r.sunday_holiday_day_overtime_factor,
        ),
        totalFactor: toDecimal(r.sunday_holiday_day_overtime_factor)!,
        payableMultiplier: toDecimal(r.sunday_holiday_day_overtime_factor)!,
        calculationMode: PayrollOvertimeCalculationMode.FULL_HOUR_FACTOR,
      },
      {
        code: 'HORA_DOM_FESTIVO_NOCTURNO',
        name: 'Hora Dom/Festivo Nocturno',
        legalPercentage: percentageFromFactor(r.sunday_holiday_night_factor),
        totalFactor: toDecimal(r.sunday_holiday_night_factor)!,
        payableMultiplier: new Prisma.Decimal(r.sunday_holiday_night_factor)
          .sub(1)
          .toString(),
        calculationMode: PayrollOvertimeCalculationMode.SURCHARGE_ONLY,
      },
      {
        ...
        name: 'Hora Extra Nocturno Dom/Festivo',
        legalPercentage: percentageFromFactor(
          r.sunday_holiday_night_overtime_factor,
        ),
        totalFactor: toDecimal(r.sunday_holiday_night_overtime_factor)!,
        payableMultiplier: toDecimal(r.sunday_holiday_night_overtime_factor)!,
        calculationMode: PayrollOvertimeCalculationMode.FULL_HOUR_FACTOR,
      },
    ];
  ...
  console.log(`Horas extra seed OK: ${rows.length * 7}`);
}

async function seedSolidarityBrackets(base: string) {
  const rows = parseCSV(path.join(base, 'solidaridad.csv'));

  const globalParameters = await prisma.payrollGlobalParameter.findMany({
  ...
  for (const globalParameter of globalParameters) {
    for (const r of rows) {
      const fromSmmlv = toDecimal(r.limite_inferior_smmlv ?? r.from_smmlv)!;
      await prisma.payrollSolidarityBracket.upsert({
        where: {
          globalParameterId_fromSmmlv: {
            globalParameterId: globalParameter.id,
            fromSmmlv,
          },
        },
  ...
  console.log(`Solidaridad seed OK: ${rows.length * globalParameters.length}`);
}
...
async function seedPayrollAccountingMappings(base: string) {
  ...
  if (businesses.length === 0) {
    console.log(
      'PayrollAccountingMapping seed omitido: no hay negocios creados',
    );
    return;
  }
  ...
  for (const business of businesses) {
    for (const r of rows) {
      ...
      if (accountCode.length === 4) {
        ...
        if (!account) {
          throw new Error(
            `Payroll accounting PUC cuenta not found: ${accountCode}`,
          );
        }
        ...
        if (!subaccount) {
          throw new Error(
            `Payroll accounting PUC subcuenta not found or inactive: ${accountCode}`,
          );
        }
      } else {
        throw new Error(
          `Payroll accounting account_code invalid: ${accountCode}`,
        );
      }
...
async function seedTaxResponsibilities() {
  const responsibilities = [
    {
      code: '05',
      name: 'Impuesto sobre la renta y complementarios régimen ordinario',
      description: 'Impuesto sobre la renta',
    },
    {
      code: '07',
      name: 'Retención en la fuente a título de renta',
      description: 'Agente retenedor',
    },
    {
      code: '10',
      name: 'Obligado a llevar contabilidad',
    ...
    {
      code: '47',
      name: 'Régimen Simple de Tributación - RST',
...
async function seedSimpleTaxRateBrackets() {
  const brackets = [
    ...
    },
    {
      groupCode: '2',
      groupName:
        'Comercio, industria, servicios técnicos, construcción, telecomunicaciones y demás actividades',
      rows: [
        ['0', '1000', '0.016000'],
        ['1000', '2500', '0.020000'],
        ['2500', '5000', '0.035000'],
    ...
    },
    {
      groupCode: '3',
      groupName:
        'Servicios profesionales, consultoría, científicos y profesiones liberales',
      rows: [
        ['0', '1000', '0.059000'],
        ['1000', '2500', '0.073000'],
        ['2500', '5000', '0.120000'],
...
async function seedSimpleTaxActivityGroupMappings() {
  const csvPath = path.join(
    ...
    'prisma',
    'seed-data',
    'simple_tax_activity_group_mapping.csv',
  );
  ...
  for (const row of rows) {
    ...
    if (!Number.isInteger(taxYear) || !ciiuCode || !groupCode) {
      throw new Error(
        `Invalid simple tax activity group mapping row: ${JSON.stringify(row)}`,
      );
    }

...
export function MovementPercentBarChart({
  items,
  headerLeft,
  headerRight,
}: MovementPercentBarChartProps) {
  ...
  return (
    <div className="h-full rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5 space-y-3">
      <div
        className={cn(
          hasHeaderControls &&
            "grid grid-cols-[2.25rem_1fr_2.25rem] items-center gap-2"
        )}
      >
        {hasHeaderControls && (
          <div className="flex justify-start">{headerLeft}</div>
        )}

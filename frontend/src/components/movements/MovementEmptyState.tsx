...
export function MovementEmptyState({ onAction }: { onAction?: () => void }) {
  return (
    <div className="rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-black/5 space-y-3">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#E6EFF5] text-[#0B3F64]">
      ...
      {onAction ? (
        <button
          type="button"
          onClick={onAction}
          className="mt-2 inline-flex items-center justify-center rounded-xl bg-[#0B3F64] px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[#0B3F64]/90 active:scale-95 transition"
        >
          Ir a contabilidad
        </button>
      ) : null}

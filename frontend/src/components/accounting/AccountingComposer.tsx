...
import type { PucNode, PucKind } from "@/src/services/puc";
...
const PUC_ITEMS: PucNode[] = [
  {
    ...
    name: "Caja",
    kind: "ASSET",
    breadcrumbs: ["Activo", "Disponible", "Caja"],
  },
  {
    code: "110505",
    name: "Caja general",
    kind: "ASSET",
    breadcrumbs: ["Activo", "Disponible", "Caja", "Caja general"],
  },
  {
    code: "1110",
    name: "Bancos",
    kind: "ASSET",
    breadcrumbs: ["Activo", "Disponible", "Bancos"],
  },
  {
    code: "1305",
    name: "Clientes",
    kind: "ASSET",
    breadcrumbs: ["Activo", "Deudores", "Clientes"],
  },
  {
    code: "2105",
    name: "Proveedores",
    kind: "LIABILITY",
    breadcrumbs: ["Pasivo", "Cuentas por pagar", "Proveedores"],
  },
  {
    code: "2365",
    name: "Retenciones",
    kind: "LIABILITY",
    breadcrumbs: ["Pasivo", "Impuestos", "Retenciones"],
  },
  {
    code: "3105",
    name: "Capital social",
    kind: "EQUITY",
    breadcrumbs: ["Patrimonio", "Capital", "Capital social"],
  },
  {
    code: "4135",
    name: "Ventas",
    kind: "INCOME",
    breadcrumbs: ["Ingresos", "Operacionales", "Ventas"],
  },
  {
    code: "4175",
    name: "Servicios",
    kind: "INCOME",
    breadcrumbs: ["Ingresos", "Operacionales", "Servicios"],
  },
  {
    code: "5105",
    name: "Gastos de personal",
    kind: "EXPENSE",
    breadcrumbs: ["Gastos", "Administración", "Personal"],
  },
  {
    code: "5205",
    name: "Arriendos",
    kind: "EXPENSE",
    breadcrumbs: ["Gastos", "Administración", "Arriendos"],
  },
];
...
function PucWizard({
  ...
  const groups = useMemo(() => {
    return uniqSorted(
      base.map((it) => crumb(it, 1)).filter(Boolean) as string[],
    );
  }, [base]);
  ...
  const accounts = useMemo(() => {
    ...
    return uniqSorted(
      scoped.map((it) => crumb(it, 2)).filter(Boolean) as string[],
    );
  }, [base, group]);
  ...
  const leafCandidates = useMemo(() => {
    ...
    return base.filter(
      (it) => crumb(it, 1) === group && crumb(it, 2) === account,
    );
  }, [base, group, account]);
  ...
  const subaccounts = useMemo(() => {
    if (!group || !account) return [];
    return uniqSorted(
      leafCandidates.map((it) => crumb(it, 3)).filter(Boolean) as string[],
    );
  ...
  function pickBestLeaf(sub: string | null) {
    if (sub) {
      return (
        leafCandidates.find((it) => crumb(it, 3) === sub) ??
        leafCandidates[0] ??
        null
      );
    }
  ...
  return (
    <div className="mt-2 rounded-2xl border border-gray-200 bg-white/70 backdrop-blur p-3">
      ...
      <div className="mt-2 flex gap-2 text-xs">
        <span
          className={`px-2 py-1 rounded-full border ${step === "KIND" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-white/70 border-gray-200 text-gray-600"}`}
        >
          1. Tipo
        </span>
        <span
          className={`px-2 py-1 rounded-full border ${step === "GROUP" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-white/70 border-gray-200 text-gray-600"}`}
        >
          2. Grupo
        </span>
        <span
          className={`px-2 py-1 rounded-full border ${step === "ACCOUNT" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-white/70 border-gray-200 text-gray-600"}`}
        >
          3. Cuenta
        </span>
        <span
          className={`px-2 py-1 rounded-full border ${step === "SUBACCOUNT" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-white/70 border-gray-200 text-gray-600"}`}
        >
          4. Subcuenta
        </span>
      </div>
      ...
      {!forcedKind && step === "KIND" && (
        <div className="mt-3 grid grid-cols-2 gap-2">
          {(
            ["ASSET", "LIABILITY", "EQUITY", "INCOME", "EXPENSE"] as PucKind[]
          ).map((k) => (
            <button
              key={k}
              type="button"
      ...
      {step === "GROUP" && (
        <div className="mt-3">
          ...
          <div className="mt-2 max-h-48 overflow-auto rounded-xl border border-gray-200 bg-white">
            ...
            {groups.length === 0 && (
              <div className="px-3 py-2 text-sm text-gray-500">Sin grupos.</div>
            )}
          </div>
      ...
      {step === "SUBACCOUNT" && (
        <div className="mt-3">
          <div className="text-xs text-gray-500">
            Elegí una subcuenta (si no hay, usás la cuenta)
          </div>
            ...
            {subaccounts.length === 0 && (
              ...
              >
                ...
                <div className="text-xs text-gray-500 truncate">
                  {kindLabel((kind ?? forcedKind) as PucKind)} &gt; {group} &gt;{" "}
                  {account}
                </div>
              </button>
            )}
      ...
      {value && (
        <div className="mt-3 text-xs text-gray-600">
          Seleccionado: <span className="font-semibold">{value.code}</span> —{" "}
          {value.name}
        </div>
      )}
    </div>
...
export function AccountingComposer({
  ...
  useEffect(() => {
    ...
    if (editingEntry.pucCode) {
      const found =
        PUC_ITEMS.find((x) => x.code === editingEntry.pucCode) ?? null;
          ...
          <div className="px-5 py-4 border-t border-white/30">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={!expanded}
                title={
                  !selectedPuc && expanded ? "Seleccioná un PUC" : undefined
                }
              >
                <svg
                  viewBox="0 0 24 24"
            ...
            {editingEntry && (
              <div className="mt-2 px-1 text-xs text-neutral-500">
                Editando:{" "}
                <span className="font-medium">{editingEntry.pucCode}</span>
                <button
                  type="button"

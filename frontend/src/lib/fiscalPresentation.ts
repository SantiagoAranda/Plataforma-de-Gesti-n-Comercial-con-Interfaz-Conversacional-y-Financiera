...
export const FISCAL_STATUS_PRESENTATION: Record<FiscalDocumentStatus, {
  ...
  LOCAL_PERSISTENCE_FAILURE: { label: "Requiere sincronizaciÃ³n local", badge: "bg-amber-100 text-amber-900", dot: "bg-amber-500", ticks: 2, tickClass: "text-slate-400" },
...
export function functionalFiscalError(status: FiscalDocumentStatus) {
  if (status === "LOCAL_PERSISTENCE_FAILURE")
    return "Factus aceptÃ³ el documento, pero no fue posible sincronizarlo localmente. Requiere revisiÃ³n; no lo reintentes.";

...
| 3 | `InventoryReferenceType` tiene `ORDER` pero se usa `ORDER_ITEM` para ventas | Baja | El valor `ORDER` de `InventoryReferenceType` no se usa en código auditado. Solo se usa `ORDER_ITEM`. Puede confundir. |
...
| Acción | Motivo | Archivos afectados/probables | Resultado esperado |
|--------|--------|------------------------------|---------------------|
| Implementar integración contable para `PURCHASE`, `ADJUSTMENT_POSITIVE`, `ADJUSTMENT_NEGATIVE` e `INVENTORY_INITIAL` | Sin esto, el activo de inventario (cuenta 1435) nunca se registra en el libro mayor | `inventory.service.ts` (agregar llamadas a `accountingService`); nuevo método en `accounting.service.ts` | Cada movimiento manual genera el asiento contable correspondiente (débito/crédito en PUC) |
| Implementar endpoint `POST /inventory/reconcile/:ingredientId` (solo admin) | Permite corregir `currentStock`/`averageCost` si quedan inconsistentes por falla parcial | `inventory.controller.ts`, `inventory.service.ts` | Herramienta de rescate sin modificar datos de producción manualmente |
| Ampliar `sales-flow.e2e-spec.ts` para verificar descuento de stock en ventas | El e2e actual no valida ningún aspecto del inventario en DB real | `backend/test/sales-flow.e2e-spec.ts` | Test verifica que `Ingredient.currentStock` disminuye tras `confirmOrder` con item SIMPLE |
| Agregar test de reversión e2e (SALE_RETURN) | Verifica la ruta HTTP real de reversión incluyendo la protección contra doble reversión | `backend/test/` (nuevo o extender existente) | Test crea orden, confirma, revierte, verifica stock; segundo intento de reversión → 409 |
| Agregar test de aislamiento `businessId` en `InventoryService` | Sin test, una regresión en filtros puede exponer datos de otros negocios | `inventory.service.spec.ts` o nuevo archivo | Confirmar que un query con `businessId` incorrecto no retorna datos de otro negocio |
| Implementar `PATCH /ingredients/:id/reactivate` en backend y botón en UI | Un ingrediente desactivado por error no puede recuperarse | `ingredients.controller.ts`, `ingredients.service.ts`; `IngredientList.tsx` (tab INACTIVE) | Operadores pueden reactivar ingredientes sin intervención de DBA |
| Eliminar código muerto (~120 líneas comentadas) en `kardex/page.tsx` | Deuda técnica de mantenimiento activo | `kardex/page.tsx` L.177-298 | Archivo limpio; sin riesgo de confundir código activo con código obsoleto |
...
| Eliminar `ORDER` del enum `InventoryReferenceType` o documentarlo | Valor definido en schema pero nunca usado en código; puede inducir a error | `schema.prisma`, `inventory.service.ts` | Enum sin valores muertos; o comentario explícito si se reserva para uso futuro |
